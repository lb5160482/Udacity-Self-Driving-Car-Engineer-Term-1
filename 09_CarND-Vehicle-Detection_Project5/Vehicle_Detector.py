...
class Vehicle_Detector():
	def __init__(self, model_dict, img_shape, windows):
		...
		self.heatmap = np.zeros((img_shape))
		self.rects_queue_length = 3
	...
	def get_cur_rects(self, img_org):
		...
		for scale in self.scale_list:
			...
			# cv2.imwrite('.\\output_images\\' + 'scale' + str(scale) + '.png', draw_img)
			# cv2.waitKey(0)
	...
	def get_cur_heat_map(self, rects):
		...
		cv2.imshow('heat', heatmap / np.max(heatmap))
		cv2.waitKey(0)
...
if __name__ == '__main__':
	...
	if platform.system() == 'Windows':
		...
		test_files = ['./test_images/full_1000960.0.png', './test_images/full_1000961.0.png',
					  ...
					  './test_images/full_1000966.0.png']
